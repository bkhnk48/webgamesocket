...
export class Node2D {
  ...
  private _theta: number = 1; 
  //public isUndisUnddefined : boolean = false;
  ...
  public setVirtualNeighbor(node: Node2D) : void{
  ...
  public setRealNeighbor(node: Node2D) {
    if (this.x + 1 == node.x && this.y == node.y) {
      this.nodeE = node;
      this.w_edge_E = 1;
    } else if (this.x == node.x && this.y + 1 == node.y) {
